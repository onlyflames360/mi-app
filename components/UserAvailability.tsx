...
const UserAvailability: React.FC<UserAvailabilityProps> = ({ user }) => {
  ...
  const [av, setAv] = useState<MonthlyAvailability>(initialAv);

  useEffect(() => {
    const fetchAv = async () => {
      const allAvs = await db.getAvailabilities();
      const existing = allAvs.find(a => a.idUsuario === user.id && a.mes === monthKey);
      if (existing) setAv(existing);
    };
    fetchAv();
  }, [user.id, monthKey]);
  ...
  const save = async (isFinal: boolean) => {
    const current = await db.getAvailabilities();
    ...
    await db.setAvailabilities([...filtered, updated]);
    setAv(updated);
