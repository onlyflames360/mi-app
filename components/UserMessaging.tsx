...
const UserMessaging: React.FC<UserMessagingProps> = ({ user }) => {
  ...
  const handleSend = () => {
    ...
    // Use async callback in setTimeout to handle database promises
    setTimeout(async () => {
      const allUsers = await db.getUsers();
      const coordinators = allUsers.filter(u => u.rol === 'coordinador').map(u => u.id);
      ...
      await db.setNotifications([newNotif, ...notifs]);
