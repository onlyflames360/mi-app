...
const UserNotifications: React.FC<UserNotificationsProps> = ({ user }) => {
  ...
  const markAsRead = async (id: string) => {
    ...
    await db.setNotifications(updated);
    await loadNotifications();
  };

  const handleCoverShift = async (notif: AppNotification) => {
    ...
    if (shift.estado !== 'en_sustitucion') {
      ...
      const allNotifs = (await db.getNotifications()).filter(n => n.id !== notif.id);
      await db.setNotifications(allNotifs);
      await loadNotifications();
      return;
    }
    ...
    const currentAllNotifs = (await db.getNotifications()).filter(n => n.refTurnoId !== notif.refTurnoId);
    await db.setNotifications([newUserNotif, oldUserNotif, coordNotif, ...currentAllNotifs]);
