...
const UserTasks: React.FC<UserTasksProps> = ({ user }) => {
  ...
  // Use useEffect to handle asynchronous database calls
  useEffect(() => {
    const fetchData = async () => {
      const fetchedShifts = await db.getShifts();
      const fetchedUsers = await db.getUsers();
      setAllShifts(fetchedShifts);
      setTasks(fetchedShifts.filter(s => s.asignadoA === user.id).sort((a, b) => a.fecha.localeCompare(b.fecha)));
      setUsers(fetchedUsers);
    };
    fetchData();
  }, [user.id]);

  // Update status asynchronously to handle database promises
  const updateStatus = async (taskId: string, status: Shift['estado']) => {
    const currentShifts = await db.getShifts();
    const updated = await Promise.all(currentShifts.map(async s => {
      if (s.id === taskId) {
        ...
        const coordAlert: AppNotification = {
          ...
          destinatarios: allUsers.filter(u => u.rol === 'coordinador').map(u => u.id),
        ...
        if (status === 'rechazado') {
          ...
          const urgent: AppNotification = {
            ...
            color: 'rojo',
            refTurnoId: s.id,
            destinatarios: allUsers.filter(u => u.rol === 'usuario' && u.id !== user.id).map(u => u.id),
            timestamp: new Date().toISOString(),
            leida: false
          };
          await db.setNotifications([urgent, coordAlert, ...notifs]);
          return { ...s, estado: 'en_sustitucion' as const };
        }

        await db.setNotifications([coordAlert, ...notifs]);
