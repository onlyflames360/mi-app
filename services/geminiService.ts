...
`;
    ...
    if (!Array.isArray(shifts)) {
      console.error('Respuesta de Gemini inválida:', response.text);
      return [];
    }
    return shifts;
  } catch (e) {
    console.error("Error en generación de planificación:", e);
    throw e;
  }
};
...
export const getStatsAnalysis = async (shifts: Shift[], users: User[]) => {
  try {
    ...
    console.error('Error en análisis de estadísticas:', e);
    return '';
  }
};
...
export const editImageWithGemini = async (base64Image: string, prompt: string) => {
  try {
    ...
    console.error('Error en edición de imagen:', e);
    return { imageUrl: '' };
  }
};
